...
def replace_abbreviations(token):
    return abbreviation_mapping.get(token, token)


# Function to tokenize a string and replace abbreviations
def tokenize(text):
    ...
    return [replace_abbreviations(token) for token in tokens]
